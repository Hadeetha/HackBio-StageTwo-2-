...
View(hb_stage_2d_1)
colnames(hb_stage_2d_1)
pathway_names <- as.character(hb_stage_2d_1$pathway)
length(pathway_names)

hb_stage_2d_1_num <- hb_stage_2d_1[, -1]
hb_2d_1_matrix <- as.matrix(hb_stage_2d_1[,-1])

length(pathway_names)
nrow(hb_2d_1_matrix)
rownames(hb_2d_1_matrix) <- pathway_names

library(pheatmap)

pheatmap(hb_2d_1_matrix, border_color = NA, color = colorRampPalette(c("firebrick", "#f0f9ff", "#084594"))(100), cluster_rows = F, cluster_cols = F, angle_col = 90, filename = "heatmap_pathway.png", width = 8, height = 6)
